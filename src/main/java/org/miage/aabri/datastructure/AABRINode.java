...
/**
 * Implémentation d'un noeud de l'AABRI. Ce noeud contient donc un fils droit, un fils gauche, une borne minimale, une
 * borne maximale et un ABRI.
 *
...
public class AABRINode extends Node {
     ...
     *
     * @return la valeur supprimée si c'est le cas, rien sinon
     *
     * @throws ValeurNonRepresenteeDansABRI dans le cas où la valeur recherchée n'est pas représentée dans l'ABRI
     *                                      courant, malgré le fait que
    ...
    public String getInfos(SimpleNode node) {
        ...
        if (node != null) {
            ...
            if ((lSon == null || lSon.isEmpty()) && rSon != null && !rSon.isEmpty()) {
                ret = node.getValue() + ":" + rSon;
            } else if ((lSon != null && !lSon.isEmpty() && (rSon == null || rSon.isEmpty()))) {
     ...
     *
     * La fonction ne se trouve pas dans la classe utils car on ne souhaite pas qu'une classe fille de AABRINode puisse
     * l'utiliser (on imagine que si
    ...
    public AABRI toAABRI(int nbreIntervalles) throws RuntimeException, ValeurDejaPresenteException, IntervalleChevauchantException,
        ...
        List<int[]> intervallesRandom = new ArrayList<>();
        ...
        for (int[] intervallesRandom1 : intervallesRandom) {
            tmp = intervallesRandom1;
