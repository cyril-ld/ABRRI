...
public class AABRI {
    ...
    /**
     * Ajoute un noeud type "ABRI" dans l'arbre binaire courant. Construit un TreeNode à partir des valeurs qui sont
     * passées en paramètre et appelle
    ...
    public boolean delete(int value) throws IntervalleInexistantException, ValeurNonRepresenteeDansABRI {
        ...
        return owner.delete(value) != null;
    }
    ...
    /**
     * Recherche un TreeNode depuis une valeur. La recherche consiste à parcourir l'AABRI pour trouver le TreeNode dont
     * l'intervalle contient la
     * valeur.
     *
    ...
    /**
     * Dans l'AABRI le noeud le plus grand est normalement celui qui se trouve tout en bas à droite de l'arbre. On
     * cherche donc à atteindre le noeud
    ...
    /**
     * Méthode en charge d'ajouter un noeud simple dans l'AABRI. La méthode parcours l'AABRI à la recherche d'un
     * intervalle qui contient la valeur à
     * ajouter. Si l'intervalle n'existe pas et si la valeur existe déjà dans l'ABRI, la méthode ne fait rien. Une fois
     * l'intervalle trouvée, la
     ...
     *
     * @return la valeur ajoutée si c'est le cas, nul sinon
     *
     * @throws IntervalleInexistantException dans le cas où aucun intervalle ne contient la valeur à ajouter
     * @throws ValeurDejaPresenteException   Si la valeur à insérer est déjà présente dans l'arbre.
     */
    ...
    /**
     * Supprime une noeud simple d'un ABRI. La méthode parcours l'AABRI à la recherche de l'intervalle contenant la
     * valeur simpleNodeValue. Si
     * l'intervalle n'existe pas, la méthode lève une exception IntervalleInexistantException. Si la valeur est
     * introuvable, la méthode renvoi null.
     * Une fois l'intervalle trouvé (et donc l'objet TreeNode), la méthode appelle la méthode TreeNode.delete(int
     * simpleNodeValue).
     *
     * Attention, lorsque le noeud ne contient plus aucune valeur, on supprime le noeud afin de ne pas garder de noeud
     * vide.
     *
     ...
     * @throws ValeurNonRepresenteeDansABRI  Si la valeur est bien dans l'intervalle de l'ABRI, mais n'est pas
     *                                       représentée
     */
    ...
    /**
     * Affiche dans un terminal une représentation telle que définie dans le README de l'abre courant. Le parcours de
     * l'AABRI est préfixe (On va au
    ...
    /**
     * Vérifie que l'ABRI contenu par le noeud courant est bien formé. Via le mécanisme des exceptions, on affiche le
     * noeud en cause si l'ABRI contenu
